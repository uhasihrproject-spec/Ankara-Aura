...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (!desktop || menuOpen || cartOpen || wishOpen || y <= 90 || delta < -4) {
        setShowKenteStrip(false);
      } else if (delta > 4) {
        setShowKenteStrip(true);
      }
  ...
  return (
    ...
    {/* ── CART OVERLAY ── */}
    <div className={`cart-overlay${cartOpen ? " is-open" : ""}`} onClick={() => { setCartOpen(false); setWishOpen(false); setShowKenteStrip(false); }} />
    <div className={`wish-overlay${wishOpen ? " is-open" : ""}`} onClick={() => { setWishOpen(false); setShowKenteStrip(false); }} />
    ...
    <div ref={cartPanelRef} className={`cart-panel${cartOpen ? " is-open" : ""}`}>
      ...
      <div className="cart-panel-head">
        ...
        <button className="cart-close" onClick={() => { setCartOpen(false); setShowKenteStrip(false); }} aria-label="Close cart">
      ...
      {cartItems.length > 0 && (
        <div className="cart-footer">
          ...
          <Link href="/checkout" className="cart-checkout-btn" onClick={() => { setCartOpen(false); setShowKenteStrip(false); }}>
            <span>Checkout — Secure Your Pieces</span>
          </Link>
          <a href="#" className="cart-continue" onClick={e => { e.preventDefault(); setCartOpen(false); setShowKenteStrip(false); }}>← continue shopping</a>
        </div>
      )}
    </div>
    ...
    <div ref={wishPanelRef} className={`wish-panel${wishOpen ? " is-open" : ""}`}>
      <div className="wish-head">
        ...
        <button className="wish-close" onClick={() => { setWishOpen(false); setShowKenteStrip(false); }} aria-label="Close wishlist">✕</button>
      </div>
      {wishlist.length === 0 ? (
        ...
        <div className="wish-list">
          {wishlist.map((item) => (
            <div key={item.slug} className="wish-item">
              ...
              <div>
                <Link href={`/shop/${item.slug}`} className="wish-remove" onClick={() => { setWishOpen(false); setShowKenteStrip(false); }}>View</Link>
      ...
      >
        <div className="nav-inner">
          ...
          <div className="nav-right">
            ...
            <button className="wish-btn" onClick={() => { setWishOpen((o) => !o); setCartOpen(false); setShowKenteStrip(false); }} aria-label={`Wishlist, ${wishlistCount} items`}>
              ♡
            ...
            <button
              ...
              onClick={() => { setCartOpen((o) => !o); setShowKenteStrip(false); }}
            ...
            <button
              ...
              onClick={() => { setMenuOpen((o) => !o); setShowKenteStrip(false); }}
    ...
    <nav className={`nav-drawer${menuOpen ? " is-open" : ""}`} aria-hidden={!menuOpen}>
      ...
      <div className="drawer-content">
        <button className="drawer-close" onClick={() => { setMenuOpen(false); setShowKenteStrip(false); }} aria-label="Close menu">
          <svg viewBox="0 0 14 14"><line x1="1" y1="1" x2="13" y2="13"/><line x1="13" y1="1" x2="1" y2="13"/></svg>
        </button>
        ...
        <ul className="drawer-links">
          {links.map((l, i) => (
            <li key={l.href}>
              <Link href={l.href} onClick={() => { setMenuOpen(false); setShowKenteStrip(false); }}>
                {l.label}
